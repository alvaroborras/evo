...
```bash
evo frontier --strategy argmax    # top node is the best committed leaf
evo status                        # baseline score + current best, for the delta
```

- The top-ranked node from `evo frontier` is the default winner. An explicit
  `exp_id` argument overrides it.
